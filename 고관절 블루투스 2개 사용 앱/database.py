...
class connectdb(QWidget):
    ...
    def call_cnt(self, table, state, date=[]):
        ...
        data ={
            ...
            'state' : state,
            'date' : [self.serialize_date(dt) for dt in date]
        }
        try:
    ...
    def call_cnt_info(self,label,search_no,env_name):
        global board_ver
        global aircell_ver
        data = {
            'env_id' : search_no
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if isinstance(result,list):
                    ...
                    save_file_name = f"{date}_{id}_{title}.xlsx"
